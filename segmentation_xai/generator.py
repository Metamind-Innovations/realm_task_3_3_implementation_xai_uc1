...
class Patient_data_generator(keras.utils.Sequence):
    ...
    def __init__(self, patient_dict, batch_size=8, image_size=512, tumor_size_low_lim=0, sl_th=None, zero_sl_ratio=1,
                 model_params={},
                 ...
                 equalize_hist=False, verbosity=False, size_eval=False, calculate_snr=False, reshape=False,
                 extract_lungs=False):
    ...
    def _load_patient(self, filename):
        ...
        if self.extract_lungs:
            ...
            if temp_args is not None:
                ...
                for k, layer in enumerate(msk):
                    ...
                    lung_mask_with_tumor_mask[k, ...] = np.array(lung_mask_with_tumor_mask[k, ...],
                                                                 np.uint8) | cv2.dilate(np.array(temp_layer, np.uint8),
                                                                                        kernel, iterations=3)
        ...
        if True:
            ...
            new_img = np.array(self._resize_3d_img(img_for_reshape, (int(img.shape[0]),
                                                                     ...
                                                                     int(temp_data_orig.GetSpacing()[1] * img.shape[
                                                                         2])), cv2.INTER_NEAREST), np.int16)

            if not self.img_only:
                new_mask = np.array(self._resize_3d_img(msk_for_reshape, (int(img.shape[0]),
                                                                          int(temp_data_orig.GetSpacing()[0] *
                                                                              img.shape[1]),
                                                                          int(temp_data_orig.GetSpacing()[1] *
                                                                              img.shape[2])), cv2.INTER_NEAREST),
                                    np.bool)
    ...
    def _load_patient_for_predict(self, filename, eval_mode=False, img_type=np.int16):
        ...
        print(img.shape)
        image_transform_params = {'original_shape': img.shape, 'normalized_shape': None, 'crop_type': None,
                                  'xy_st': None,
                                  'xy_end': None, 'z_st': None, 'z_end': None, 'img_origin': temp_data.GetOrigin(),
    ...
    def _normalization_init(self):
        if self.normalize and len(self.norm_coeff) != 2 and not self.predict:
            ...
            mean = np.mean(
                [np.mean(self._load_patient_for_predict(filename, eval_mode=True)[0].flatten()) for filename in
                 tqdm(self.filenames)])
            std = np.mean([np.std(self._load_patient_for_predict(filename, eval_mode=True)[0].flatten()) for filename in
                           tqdm(self.filenames)])
    ...
    def _resample_intensities(self, img):
        ...
        for st in range(self.step, max_val_img + self.step, self.step):
            ...
            v_count += 1
        if self.verbosity:
            print('Amount of unique values, original img: ', len(np.unique(img.flatten())), 'resampled img: ',
        ...
        if not self.predict:
            ...
            for pat in tqdm(self.filenames):
                ...
                if self._slice_thickness and int(temp_img.GetSpacing()[2]) != self._slice_thickness:
                    ...
                    tum_indicies = np.where(z_tumor_distr > self._tum_size / np.prod([*temp_msk_im.GetSpacing()][:2]))[
                        0]
                    ...
                    if size_eval:
                        temp_distr = list(filter(lambda x: x > 0,
                                                 np.round(z_tumor_distr * np.prod([*temp_msk_im.GetSpacing()][:2]),
                                                          decimals=1)))  # mm^2
                        self.tum_size_distr += temp_distr
                        self.tum_volume_distr[self.patient_dict[pat][0].split('\\')[-2]] = np.round(
                            np.sum(z_tumor_distr) * temp_msk_im.GetSpacing()[0] * temp_msk_im.GetSpacing()[1] *
                            temp_msk_im.GetSpacing()[2] * 0.001, 2)  # ml
    ...
    def __len__(self):
        if self.predict:
            ...
            if self.verbosity:
                print('-' * 7,
                      'Generator contained %d tumor slices and %d empty slices' % (len_tum_dict, len_zero_dict),
                      '-' * 7)
