...
class GenerateResultBox(ResultSet):
    ...
    def get_stats_with_ci(self, label: str = None, nsamples: int = 2000) -> DataFrame:
        ...
        if self._threshold == -1:
            self.get_optimal_threshold()
        y_pred_binary = (np.array(y_pred) > self._threshold).astype(int)
        dict_results["balanced accuracy"] = self._get_ci(y_label, y_pred_binary,
                                                         sklearn.metrics.balanced_accuracy_score, nsamples)
    ...
    def print_confusion_matrix(self, label: str = None, class_names: list = None,
        ...
        if save_fig:
            if normalize:
                plt.savefig(
                    "normalized confusion matrices on the " + label + " " + class_names[0] + " vs " + class_names[
                        1] + " .png",
                    dpi=300)
            else:
                plt.savefig(
                    "confusion matrices on the " + label + " " + class_names[0] + " vs " + class_names[1] + " .png",
                    dpi=300)
        return fig
