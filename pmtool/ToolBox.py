        ...
        if self._data_type == 'dcm':
            ...
            for pat, path in tqdm(self, desc='Patients processed'):
                ...
                for i, temp_slice in enumerate(image):
                    new_row = pd.Series([pat, str(i), *[self.__val_check(temp_slice, x) for x in params_list]],
                                        index=['patient', 'slice#', *params_list])
                    dataset_stats = pd.concat([dataset_stats, new_row.to_frame().T], ignore_index=True)
