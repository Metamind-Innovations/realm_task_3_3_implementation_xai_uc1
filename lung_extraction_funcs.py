...
def create_flipped_mask(image, i_orig, kernel, method=0):  # image is a lung mask image, i_orig is an image volume
    ...
    if method:
        ...
        for i in range(min_bound, min_bound + 5):
            ...
            lungs_center = int(rib_view.shape[0] / 2 - c_limit) + np.where(
                filtered_distr[int(rib_view.shape[0] / 2 - c_limit):int(rib_view.shape[0] / 2 + c_limit)] == np.max(
                    filtered_distr[int(rib_view.shape[0] / 2 - c_limit):int(rib_view.shape[0] / 2 + c_limit)]))[0][0]
    ...
    for i in range(min_bound, max_bound + 1):
        ...
        if lungs_center_ind[i] > int(image.shape[2] / 2):
            ...
            part2 = np.hstack(
                (ax[:, lungs_center_ind[i]:], np.zeros((image.shape[1], 2 * lungs_center_ind[i] - image.shape[2]))))
            ...
            part1 = np.hstack(
                (np.zeros((image.shape[1], image.shape[2] - 2 * lungs_center_ind[i])), ax[:, :lungs_center_ind[i]]))
                        ...
                        for imgfile in files:
                            if re.search('volume', imgfile.lower()) or re.search('image', imgfile.lower()):
                                ...
                                Patient_dict[i] = [os.path.join(general_path, patient, temp_img),
                                                   os.path.join(general_path, patient, temp_mask)]
                                i += 1
                                temp_mask = ''
                                temp_img = ''
                else:
